# WhatsApp Chatbot - Baileys + LangChain + OpenRouter

An intelligent WhatsApp chatbot built with **Baileys**, **LangChain**, **OpenRouter**, **RAG**, and per-user memory.

## Project Title

**WhatsApp Chatbot** - A conversational AI bot for WhatsApp using Baileys for transport, LangChain for workflow orchestration, OpenRouter as the LLM provider, a custom knowledge base for RAG, and per-user personal memory.

## Features

- **WhatsApp messaging** through Baileys (QR code authentication)
- **LangChain-based AI workflow** with intelligent routing
- **OpenRouter LLM** integration (supports multiple models via a single API)
- **RAG (Retrieval-Augmented Generation)** with custom knowledge base
- **Conversational memory** per WhatsApp user
- **Personal information extraction** from user messages
- **Per-user isolation** - each WhatsApp JID has separate memory and context
- **Session persistence** - authentication saved in `auth/creds.json`
- **Graceful error handling** - LLM/RAG errors don't crash the bot
...
WhatsApp
   ↓
Baileys (socket client)
   ↓
Workflow (orchestration)
   ├── Memory (per-user InMemoryChatMessageHistory + facts map)
...
| File | Responsibility |
|------|----------------|
| `src/index.ts` | Application startup - calls `startWhatsAppClient()` |
| `src/whatsapp.ts` | Baileys WhatsApp connection + message I/O. Handles QR auth, reconnection, message sending |
| `src/llm.ts` | Model configuration: `getChatModel()`, initialization via `OPENROUTER_API_KEY`/`OPENROUTER_MODEL` |
| `src/rag.ts` | Knowledge-base: `retrieveContext()`, loads `knowledge.txt`, embeddings, vector store, cosine similarity search |
| `src/memory.ts` | Per-user memory: `getPersonalFacts()`, `addMemory()`, conversation history via `InMemoryChatMessageHistory`. Re-exports `extractPersonalFacts`, `isFactAbout` from `extraction.ts` |
| `src/workflow.ts` | AI orchestration: detects memory queries vs RAG questions, routes to correct path, orchestrates extraction + RAG + LLM |
| `src/extraction.ts` | LangChain-based fact extraction from user messages. Returns `{"facts": [{"key", "value"}, ...]}` |
| `knowledge/knowledge.txt` | Custom knowledge base for RAG (21 lines about LangChain concepts) |
...
2. Edit `.env` with your OpenRouter credentials:
...
- `OPENROUTER_API_KEY`: Your OpenRouter API key (get it from [openrouter.ai/dashboard/keys](https://openrouter.ai/dashboard/keys))
- `OPENROUTER_MODEL`: The model to use (e.g., `anthropic/claude-3.5-sonnet`, `openai/gpt-4o`, `google/gemini-pro`). Defaults to `anthropic/claude-3.5-sonnet` if omitted.

**Never commit `.env` to version control**. The `.gitignore` file excludes it.
...
2. On first run: outputs `Waiting for QR code scan...` and displays a QR code
...
5. On subsequent runs, the bot restores the saved session automatically

### First-startup behavior

1. The bot outputs: `Waiting for QR code scan...`
2. A QR code appears in the console
3. Scan the QR code with your WhatsApp app
4. Once scanned, the session is saved and the bot reconnects automatically
5. If the session expires or `auth/creds.json` is deleted, the QR code prompt appears again

### WhatsApp Authentication

- Baileys uses **QR code authentication** for initial setup
- Session data is stored in `auth/creds.json` (git-ignored)
...
- The next message processed will re-initialize RAG and use the updated content
...
```text
User: What do you know about me? (New user)
...
| **Invalid model** | Check `OPENROUTER_MODEL` in `.env`. Supported models depend on your OpenRouter plan. |
| **OpenRouter/API errors** | The workflow catches LLM errors and sends "Sorry, I encountered an error processing your message." to WhatsApp. The process does **not** crash. |
...
- **Never expose API keys** in logs, documentation, or source code
- **Never commit Baileys authentication/session files** - `auth/creds.json` is git-ignored
- Use `.env.example` for configuration documentation only (placeholder values)
- The `.env` file is listed in `.gitignore`
- Keep your OpenRouter API key confidential - it enables LLM calls
- Do not share `auth/creds.json` or session data
